...
{    
    float ballsradii=radius+0.2;
    Vector3 axis=handle_rotationaxis.getOrentiation();
    axis.Normalize();
	float ballwhight = handle_mass*0.8;
    if (thetype == goalkeeper)
    {
        balls.push_back(new Ball(ballsradii, handle_center + axis*TABLE_WIDTH/12, ballwhight,handle_rotationaxis,false,frt, 0,this));
    ...
    {
        balls.push_back(new Ball(ballsradii, handle_center + axis*(TABLE_WIDTH/6)+HALF_Player_WIDTH+ballsradii, ballwhight,handle_rotationaxis,false,frt, 0,this));
	...
	{
        balls.push_back(new Ball(ballsradii, handle_center + axis*(TABLE_WIDTH/4)+HALF_Player_WIDTH+ballsradii, ballwhight,handle_rotationaxis,false,frt, 0,this));
	...
	{
        balls.push_back(new Ball(ballsradii, handle_center + axis*(TABLE_WIDTH/3)+HALF_Player_WIDTH+ballsradii, ballwhight,handle_rotationaxis,false,frt, 0,this));
        players.push_back(new Player(handle_center + axis*TABLE_WIDTH/6, 10, handle_rotationaxis,false,frt, this));
...
void Handle::fillconstants(Wall *w) const
{
	for (unsigned int i = 0; i<balls.size(); i++)
    {
         Body::StaticFrictionCoffeciants.insert(pair<BodyPair,float>(BodyPair(balls[i],w),SFC_HANDLE_WALL));
         Body::RestCoffeciants.insert(pair<BodyPair,float>(BodyPair(balls[i],w),RC_HANDLE_WALL));
         Ball::RollingFrictionCoffs.insert(pair<BodyPair, float>(BodyPair(balls[i], w), ROFC_HANDLE_WALL));
    }
}
...
void Handle::stopRotation(const bool &applyonorigin)
{
    Body::stopRotation(applyonorigin);
    for (unsigned int i = 0; i<players.size(); i++)
        players[i]->stopRotation(false);
    for (unsigned int i = 0; i<balls.size(); i++)
        balls[i]->stopRotation(false);
}
void Handle::stopMovement(const bool &applyonorigin)
{
    Body::stopMovement(applyonorigin);
    for (unsigned int i = 0; i<players.size(); i++)
        players[i]->stopMovement(false);
    for (unsigned int i = 0; i<balls.size(); i++)
        balls[i]->stopMovement(false);
}
